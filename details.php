...
if($client_id==0)
    {
	  $a=mysqli_query ($link,"INSERT INTO clients (name, dob, sex, tel, insurance, percentage, date,  period,  user, status) VALUES ('$fullname', '$dob', '$sex', '$tel', '$insurance', '$percentage', '$date', '$period', '$user', 1)"); 
      if(!$a){ die("Sorry, could not insert data: ".mysqli_error($link)); }
    }
	
	if($client_id>0)
    {
	  $b=mysqli_query ($link,"UPDATE clients SET name='$fullname', dob='$dob', sex='$sex', tel='$tel', insurance='$insurance', percentage='$percentage', date='$date', period='$period', user='$user', status=1 WHERE client_id=$client_id");	  
   ...
   <div style="width:99.5%;opacity:0.9;background-color:#FFF;position:absolute;top:54px;padding:2px;margin:1px 0;border: 1px solid #000; -webkit-border-radius:5px; box-shadow: 0px 0px 20px #000; height: 780px; overflow:auto;">
      ...
      <div  style="width: 99%; height: 690px; border: 0px solid #09F; position: absolute; background-color: #fff; left: 5px; top: 77px;">
         <!-- <a style="color:white;font-size:24px;" href="find.php"  onclick="return hs.htmlExpand(this, { objectType:'iframe'} )">
            <div style="position:absolute;border-radius:87px 9px 0px 0px;border-bottom:0px;width:430px;border-top:1px solid #09F;border-left:1px solid #09F;border-right:1px solid #09F;height:44px;background-color:#999;left:1290px;top:23px;z-index:1;">
